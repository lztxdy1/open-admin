...
CREATE TABLE sys_oper_log (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  title VARCHAR(64), business_type VARCHAR(64), method VARCHAR(255), request_method VARCHAR(16), oper_name VARCHAR(64), oper_ip VARCHAR(64), oper_url VARCHAR(255), oper_param CLOB, json_result CLOB, status INT, error_msg CLOB, oper_time TIMESTAMP
);
CREATE TABLE sys_login_log (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  username VARCHAR(64), ipaddr VARCHAR(64), status VARCHAR(16), msg VARCHAR(255), login_time TIMESTAMP
);
CREATE TABLE sys_user_role (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, user_id BIGINT, role_id BIGINT);
CREATE TABLE sys_role_menu (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, role_id BIGINT, menu_id BIGINT);
...
CREATE TABLE sys_message (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  title VARCHAR(128), content CLOB, receiver VARCHAR(64), read_flag INT DEFAULT 0, create_by VARCHAR(64), create_time TIMESTAMP
);
